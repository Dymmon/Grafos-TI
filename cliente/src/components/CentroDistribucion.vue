<template>
  <v-container>
    <h3 class="headline mb-8">Centros de distribución</h3>
    <v-spacer></v-spacer>
    <v-row>
      <v-col cols="6"><h3 class="title-2">Número de camiones</h3></v-col>
      <v-col cols="2">
    ...
    <v-row v-for="(centro, i) in centros" :key="i">
      <v-col cols="3"
        ><h3>CD{{ centro.id }}</h3></v-col
      >
      <v-col cols="4">
        <v-select
          :items="camiones"
